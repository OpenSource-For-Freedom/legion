...
fn render_header(f: &mut Frame, area: Rect, app: &App) {
    ...
    let alert_color = if alert_count == 0 {
        Color::Green
    } else {
        CRIT_COLOR
    };
    ...
    let qua_span = if app.feed_info.quarantine_count > 0 {
        Span::styled(
            format!("⚑ {} QUARANTINED", app.feed_info.quarantine_count),
...
fn render_alerts_table(f: &mut Frame, area: Rect, app: &App) {
    ...
    let all_rows: Vec<Row> = if app.alerts.is_empty() {
        vec![Row::new(vec![
            ...
            Cell::from(""),
            Cell::from(""),
            Cell::from(" ● No active alerts — system clear")
...
fn render_alert_detail(f: &mut Frame, area: Rect, app: &App) {
    let lines = if let Some(a) = app.table_state.selected().and_then(|i| app.alerts.get(i)) {
        let cves = if a.cve_ids.is_empty() {
            "—".to_owned()
        } else {
            a.cve_ids.join(", ")
        };
    ...
    let lines = vec![
        ...
        Line::from(vec![
            ...
            Span::styled(
                ...
                Style::default().fg(if app.feed_info.active_connections.is_empty() {
                    Color::Green
                } else {
                    Color::Yellow
                }),
...
fn gauge_line(label: &str, pct: f32, color: Color) -> Line<'static> {
    ...
    let bar = format!(
        "{}{}",
        "█".repeat(filled.min(bar_total)),
        "░".repeat(bar_total - filled.min(bar_total))
    );
    Line::from(vec![
...
fn render_feed_info(f: &mut Frame, area: Rect, app: &App) {
    ...
    let lines = vec![
        Line::from(vec![
            ...
            Span::styled(
                fi.events_cached.to_string(),
                Style::default()
                    .fg(if fi.events_cached > 0 { ACCENT } else { DIM })
                    .add_modifier(Modifier::BOLD),
            ),
        ]),
        Line::from(vec![
            ...
            Span::styled(
                fi.ips_cached.to_string(),
                Style::default()
                    .fg(if fi.ips_cached > 0 {
                        Color::LightRed
                    } else {
                        DIM
                    })
        ...
        Line::from(vec![
            ...
            Span::styled(
                fi.quarantine_count.to_string(),
                Style::default()
                    .fg(if fi.quarantine_count > 0 {
                        Color::LightRed
                    } else {
                        Color::Green
                    })
...
fn render_scan_status(f: &mut Frame, area: Rect, app: &App) {
    ...
    let lines = vec![
        Line::from(vec![
            ...
            Span::styled(
                si.last_scan.as_deref().unwrap_or("never").to_string(),
                Style::default().fg(Color::White),
            ),
        ]),
        Line::from(vec![
        ...
        conns
            ...
            .map(|ip| {
                let blacklisted = app
                    .alerts
                    .iter()
                    .any(|a| a.ip_address.as_deref() == Some(ip.as_str()));
                if blacklisted {
                    Line::from(vec![
...
fn fmt_kb(kb: u64) -> String {
    if kb < 1024 {
        ...
        format!("{:.1} MB/s", kb as f64 / 1024.0)
    }
}
