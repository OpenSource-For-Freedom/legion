"""
Orchestrator — single end-to-end Legion Dev LoRA run, single-flight + time-boxed.
...
def parse_args(argv=None):
    p = argparse.ArgumentParser("legion_dev.run")
    p.add_argument("--tier", default=os.environ.get("LEGION_DEV_TIER", DEFAULT_TIER))
    p.add_argument("--time-budget-min", type=float, default=_envf("LEGION_DEV_TIME_BUDGET_MIN", 30.0))
    p.add_argument("--steps", type=int, default=_envf("LEGION_DEV_STEPS", 200))
    p.add_argument("--rank", type=int, default=_envf("LEGION_DEV_RANK", 32))
    p.add_argument("--max-examples", type=int, default=_envf("LEGION_DEV_MAX_EXAMPLES", 256))
    p.add_argument("--instructions-per", type=int, default=_envf("LEGION_DEV_INSTRUCTIONS_PER", 3),
                   help="verified solutions to sample per training task")
    p.add_argument("--teacher", choices=["hybrid", "model", "template"],
                   default=os.environ.get("LEGION_DEV_TEACHER", "hybrid"))
    p.add_argument("--model", default=os.environ.get("LEGION_DEV_TEACHER_MODEL", "qwen2.5-coder:7b"))
...
def main(argv=None) -> int:
    ...
    candidate_tag = args.candidate_tag or f"legion-dev:{size}-{run_id}"
    cfg = {"run_id": run_id, "tier": args.tier, "candidate_tag": candidate_tag,
           ...
           "instructions_per": args.instructions_per, "teacher": args.teacher, "stages": sorted(stages),
    ...
    try:
        with Lock(TRAINING_ROOT / ".work" / "lora.lock", args.force):
            if "dataset" in stages:
                ds_stats = dataset_mod.build_dataset(data_dir, instructions_per=args.instructions_per,
            ...
            if "eval" in stages:
                ...
                try:
                    cand_eval = eval_mod.evaluate_model(eval_target, tier=args.tier)
                    runlog.log("eval", "OK", f"{eval_target} pass@1 {cand_eval.passed}/{cand_eval.n}",
                               run_id=run_id, gates=cand_eval.gates_cleared, pass_rate=round(cand_eval.pass_rate, 3))
                except Exception as e:
                    runlog.log("eval", "FAIL", f"{e}", run_id=run_id)
                if build_result and build_result.status == "ok" and candidate_tag != args.model:
                    try:
                        base_eval = eval_mod.evaluate_model(args.model, tier=args.tier)
                    except Exception:
                        pass
    ...
    if status == "ok" and train_result and train_result.status == "partial":
        status = "partial"
    if status == "ok" and build_result and build_result.status == "error":
        status = "partial"
